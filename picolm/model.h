...
typedef struct {
    ...
    void *post_attn_norm_bias_dev[MAX_LAYERS]; /* device copy (GPT-2) */
    ...
    void *ffn_up_bias_dev[MAX_LAYERS];     /* GPT-2: [n_ffn] F32 */
    void *ffn_down_bias_dev[MAX_LAYERS];   /* GPT-2: [dim] F32 */
    ...
    int n_gpu_layers;                     /* -ngl: layers 0..n-1 on GPU, rest on CPU (0=-1=all) */
} gpu_weights_t;
#endif /* PICOLM_GPU */
