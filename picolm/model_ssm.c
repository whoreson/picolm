...
/* Benchmark: GPU prefill path needs the same per-layer timing as CPU prefill.
 * The callback and user data are defined in model_core.c. */
extern bench_layer_cb_t g_bench_cb;
...
float *model_forward_gpu(model_t *m, int token, int pos) {
     ...
     * Keeps activations on-device across all layers.
     * Falls back to model_forward() if pipeline not ready. */    model_config_t *c = &m->config;
...
after_qkv:
         ...
         * NOT strided at xb_stride. Use non-strided variant on all platforms. */        picolm_gpu_kv_store_dev_batched(1, attn_ord, start_pos, n_ubatch,
     ...
     * output has meaningful timing data. GPU prefill records all commands
     * instantly; the actual work happens during the fence wait. */
...
float *model_forward_prefill_gpu(model_t *m, const int *tokens, int n_tokens, int start_pos, volatile int *interrupt) {
    ...
    int last_ubatch_size = n_tokens;
    {
        ...
        while (offset < n_tokens) {
            ...
            int ubatch_rc = _prefill_gpu_ubatch(m, s, gw, gpu_dev,
                ...
                this_ubatch, ubatch_start_pos, interrupt);            if (ubatch_rc) {
                free(host_embd);
                return NULL;
            }
