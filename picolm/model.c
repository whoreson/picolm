...
static void ssm_prefill_layer(model_t *m, run_state_t *s,
    ...
    int n_k = c->ssm_n_group;
    int n_vpk = n_v_heads / n_k;
    int do_remap = !m->from_safetensors && n_k > 0 && n_k < n_v_heads && (n_vpk / 2) > 0;
    int value_dim = c->ssm_d_inner;
    int xb2_stride = (value_dim > dim) ? value_dim : dim;
    int qk_dim = d_state * n_k_heads;
    int repeat = n_v_heads / n_k_heads;
    ...
    /* Allocate per-token scratch for QKV + Z + conv output */
...
#pragma omp parallel for
    ...
    for (int bi = 0; bi < n_tokens; bi++)
        rmsnorm(xb_batch + bi * dim, x_batch + bi * dim, s->attn_norm_w[l], dim, eps);
...
#pragma omp parallel for
    ...
    /* 6. Alpha + gate_exp + Beta (per token, batched across tokens) */
    float *ge_batch = (float *)malloc((size_t)n_tokens * n_v_heads * sizeof(float));
    float *beta_batch = (float *)malloc((size_t)n_tokens * n_v_heads * sizeof(float));
    {
        gguf_type_t alpha_type = lw->type_ssm_alpha;
        size_t alpha_rb = gguf_type_row_size(alpha_type, dim);
        gguf_type_t beta_type = lw->type_ssm_beta;
        size_t beta_rb = gguf_type_row_size(beta_type, dim);
#ifdef _OPENMP
#pragma omp parallel for
#endif
        for (int bi = 0; bi < n_tokens; bi++) {
            float *xb = xb_batch + bi * dim;
            float *ge = ge_batch + bi * n_v_heads;
            float *bet = beta_batch + bi * n_v_heads;
            for (int h = 0; h < n_v_heads; h++) {
                int gh = do_remap ? qwen35_vhead_gguf(h, n_vpk, n_k) : h;
                float a = vec_dot((const uint8_t *)lw->ssm_alpha + (size_t)gh * alpha_rb,
                                  xb, dim, alpha_type) + s->ssm_dt_w[l][h];
                float sp = (a > 20.0f) ? a : (a < -20.0f) ? expf(a) : logf(1.0f + expf(a));
                float gate = sp * s->ssm_a_w[l][h];
                ge[h] = (gate < -50.0f) ? 0.0f : expf(gate);
                float b = vec_dot((const uint8_t *)lw->ssm_beta + (size_t)gh * beta_rb,
                                  xb, dim, beta_type);
                bet[h] = 1.0f / (1.0f + expf(-b));
            }
        }
    }

    /* 7. Sequential state update (THE HOT PATH) - AVX-512 vectorized
     * Uses qkv_batch space as scratch for q_rep/k_rep/sk/dv/output
     * Total needed per token: d_state*n_v_heads*5 = 128*48*5 = 30720 floats
     * qkv_batch has conv_dim = 10240 per token. Need more scratch.
     * Use a separate buffer. */
    {
        int scratch_per_tok = d_state * n_v_heads * 5;
        float *scratch = (float *)malloc((size_t)n_tokens * scratch_per_tok * sizeof(float));
        for (int bi = 0; bi < n_tokens; bi++) {
            float *conv = conv_batch + bi * conv_dim;
            float *q = conv;
            float *k = conv + qk_dim;
            ...
            /* Scratch layout per token: [q_rep | k_rep | sk | dv | sout] */
            float *q_rep = scratch + (size_t)bi * scratch_per_tok;
            float *k_rep = q_rep + d_state * n_v_heads;
            float *sk = k_rep + d_state * n_v_heads;
            float *dv = sk + d_state * n_v_heads;
            float *ssm_output = dv + d_state * n_v_heads;

            /* Repeat q/k to n_v_heads (GQA-style: h / repeat) */
            for (int h = 0; h < n_v_heads; h++) {
                int kh = h / repeat;
...
#ifdef PICOLM_AVX512
            for (int h = 0; h < n_v_heads; h++) {
                __m512 g = _mm512_set1_ps(ge[h]);
                float *st_h = state + h * d_state * d_state;
                for (int d1 = 0; d1 < d_state; d1++) {
                    float *row = st_h + d1 * d_state;
                    for (int d2 = 0; d2 < d_state; d2 += 16) {
                        __m512 s = _mm512_loadu_ps(row + d2);
                        _mm512_storeu_ps(row + d2, _mm512_mul_ps(s, g));
                    }
                }
            }
#else
            for (int h = 0; h < n_v_heads; h++) {
                float ge_h = ge[h];
                float *st_h = state + h * d_state * d_state;
                for (int i = 0; i < d_state * d_state; i++) st_h[i] *= ge_h;
            }
#endif

            /* sk[d2,h] = sum_{d1}(state[h][d1*d_state+d2] * k[h][d1]) */
#ifdef PICOLM_AVX512
            for (int h = 0; h < n_v_heads; h++) {
                float *st_h = state + h * d_state * d_state;
                __m512 kv[8];
                for (int d1 = 0; d1 < d_state; d1 += 16)
                    kv[d1/16] = _mm512_loadu_ps(k_rep + h * d_state + d1);
                for (int d2 = 0; d2 < d_state; d2++) {
                    __m512 s = _mm512_setzero_ps();
                    for (int d1 = 0; d1 < d_state; d1 += 16) {
                        __m512 st_d = _mm512_set1_ps(st_h[d1 * d_state + d2]);
                        s = _mm512_fmadd_ps(st_d, kv[d1/16], s);
                    }
                    sk[d2 * n_v_heads + h] = _mm512_reduce_add_ps(s);
                }
            }
#else
            for (int h = 0; h < n_v_heads; h++) {
            ...
            /* dv[d,h] = (v[h*head_v_dim+d] - sk[d*n_v_heads+h]) * beta[h] */
            for (int d = 0; d < d_state; d++)
                for (int h = 0; h < n_v_heads; h++)
                    dv[d * n_v_heads + h] = (v[h * head_v_dim + d] - sk[d * n_v_heads + h]) * bet[h];

            /* State update: st_h[d1*d_state+d2] += k[h][d1] * dv[d2*n_v_heads+h] */
#ifdef PICOLM_AVX512
            for (int h = 0; h < n_v_heads; h++) {
                float *st_h = state + h * d_state * d_state;
                for (int d1 = 0; d1 < d_state; d1++) {
                    float kf = k_rep[h * d_state + d1];
                    float *st_row = st_h + d1 * d_state;
                    for (int d2 = 0; d2 < d_state; d2++)
                        st_row[d2] += kf * dv[d2 * n_v_heads + h];
                }
            }
#else
            for (int h = 0; h < n_v_heads; h++) {
            ...
            /* Output: sout[d,h] = sum_{d1}(state[h][d1][d] * q[h][d1]) */
#ifdef PICOLM_AVX512
            for (int h = 0; h < n_v_heads; h++) {
                float *st_h = state + h * d_state * d_state;
                __m512 qv[8];
                for (int d1 = 0; d1 < d_state; d1 += 16)
                    qv[d1/16] = _mm512_loadu_ps(q_rep + h * d_state + d1);
                for (int d2 = 0; d2 < d_state; d2++) {
                    __m512 s = _mm512_setzero_ps();
                    for (int d1 = 0; d1 < d_state; d1 += 16) {
                        __m512 st_d = _mm512_set1_ps(st_h[d1 * d_state + d2]);
                        s = _mm512_fmadd_ps(st_d, qv[d1/16], s);
                    }
                    ssm_output[d2 * n_v_heads + h] = _mm512_reduce_add_ps(s);
                }
            }
#else
            for (int h = 0; h < n_v_heads; h++) {
...
#pragma omp parallel for
    ...
    if (do_remap) {
        for (int bi = 0; bi < n_tokens; bi++) {
            ...
            float *fo_g = alloca(value_dim * sizeof(float));
            for (int h = 0; h < n_v_heads; h++) {
                int gh = qwen35_vhead_gguf(h, n_vpk, n_k);
                memcpy(fo_g + gh * head_v_dim, fo + h * head_v_dim, head_v_dim * sizeof(float));
            }
            matmul(xb2_batch + bi * xb2_stride, fo_g, lw->ssm_out, value_dim, dim, lw->type_ssm_out);
        }
    } else {
        tensor_set_repacked(m->repack_used[ri+5] ? m->repack_buffers[ri+5] : NULL);
        matmul_batch(xb2_batch, xb2_batch, n_tokens, lw->ssm_out, value_dim, dim, lw->type_ssm_out);
        tensor_set_repacked(NULL);
    }
...
#endif
        for (int bi = 0; bi < n_tokens; bi++)
            rmsnorm(xb_batch + bi * dim, x_batch + bi * dim, s->post_attn_norm_w[l], dim, eps);
...
#pragma omp parallel for
    ...
    for (int l = 0; l < c->n_layers; l++) {
        ...
        if (c->has_ssm && !lw->is_attn_layer) {
            /* SSM layer: batched prefill with batched projections */
            ssm_prefill_layer(m, s, x_batch, xb_batch, xb2_batch,
                              hb_batch, hb2_batch,
                              lw, l, n_tokens, start_pos);
            continue;
        }
