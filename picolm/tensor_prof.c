...
/* Profiling: track time spent in matmul_batch (PICOLM_PROFILE=1) */
...
static struct {
    double t_f32_gemm, t_q8_d, t_q4_d, t_q5_d, t_q4k, t_q6k, t_q3k, t_q5k,
           t_q4_0_44, t_q4_0_48, t_scalar_parallel, t_scalar_single, t_other;
    int    n_f32_gemm, n_q8_d, n_q4_d, n_q5_d, n_q4k, n_q6k, n_q3k, n_q5k,
           n_q4_0_44, n_q4_0_48, n_scalar_parallel, n_scalar_single, n_other;
} picolm_mm_prof;

static void picolm_mm_prof_print(void) {
    fprintf(stderr, "\n=== matmul_batch profile ===\n");
    #define P(x) if(picolm_mm_prof.n_##x) fprintf(stderr, "%14s: %6d calls, %6.2f ms (%5.1f%%)\n", #x, picolm_mm_prof.n_##x, picolm_mm_prof.t_##x*1000, picolm_mm_prof.t_##x/(picolm_mm_prof.t_f32_gemm+picolm_mm_prof.t_q8_d+picolm_mm_prof.t_q4_d+picolm_mm_prof.t_q5_d+picolm_mm_prof.t_q4k+picolm_mm_prof.t_q6k+picolm_mm_prof.t_q3k+picolm_mm_prof.t_q5k+picolm_mm_prof.t_q4_0_44+picolm_mm_prof.t_q4_0_48+picolm_mm_prof.t_scalar_parallel+picolm_mm_prof.t_scalar_single+picolm_mm_prof.t_other)*100)
    P(f32_gemm); P(q8_d); P(q4_d); P(q5_d); P(q4k); P(q6k); P(q3k); P(q5k);
    P(q4_0_44); P(q4_0_48); P(scalar_parallel); P(scalar_single); P(other);
    fprintf(stderr, "===========================\n\n");
    #undef P
}
...
void matmul_batch(float *out, const float *x, int n_batch,
                   const void *W, int n, int d, gguf_type_t qtype) {
    double t0 = getenv("PICOLM_PROFILE") ? picolm_now() : 0;
#ifdef PICOLM_GPU
    ...
    if (picolm_sgemm(d, n_batch, n, wptr, n, x, n, out, d, qtype, GGUF_TYPE_F32, 0, 1)) {
        if (t0) { double dt = picolm_now() - t0; picolm_mm_prof.t_f32_gemm += dt; picolm_mm_prof.n_f32_gemm++; }
        return;
    }
    ...
    if (have_qx && qx_d_buf && d >= 4 &&
        ...
        if (n_batch >= min_batch) {
            ...
            tensor_parallel_for(nth, qgemm_d_task, &ctx);
            if (t0) { double dt = picolm_now() - t0;
                if (qtype == GGUF_TYPE_Q8_0) { picolm_mm_prof.t_q8_d += dt; picolm_mm_prof.n_q8_d++; }
                else if (qtype == GGUF_TYPE_Q4_0) { picolm_mm_prof.t_q4_d += dt; picolm_mm_prof.n_q4_d++; }
                else { picolm_mm_prof.t_q5_d += dt; picolm_mm_prof.n_q5_d++; }
            }
            if (qx_buf) { free(qx_buf); if (qx_d_buf) free(qx_d_buf); }
            return;
        }
    ...
    if (n_threads <= 1 || d < 4 || d < matmul_min_rows) {
        ...
        if (t0) { double dt = picolm_now() - t0; picolm_mm_prof.t_scalar_single += dt; picolm_mm_prof.n_scalar_single++; }
        if (qx_buf) { free(qx_buf); if (qx_d_buf) free(qx_d_buf); }
        return;
    }
    ...
    matmul_worker_f(&pool_tasks[0]);
    pool_wait(nt);
    if (t0) { double dt = picolm_now() - t0; picolm_mm_prof.t_scalar_parallel += dt; picolm_mm_prof.n_scalar_parallel++; }
    if (qx_buf) { free(qx_buf); if (qx_d_buf) free(qx_d_buf); }
    if (getenv("PICOLM_PROFILE") && n_batch > 0) picolm_mm_prof_print();
}
